...
const StyledFlashCardSideContent = styled.div<{ $noPadding?: boolean; }>`
    &.card-side-content {
        ${props => props.$noPadding ? 'padding: 0;' : 'padding: var(--spacing-sm);'}
        flex: 1;
        display: flex;
        align-items: center;
...
export const FlashCardBoardItem = ({
    ...
    return <>
        <StyledFlashCardBoardItem
            className="flash-card-board-item"
            size="small"
            title={`Card #${index + 1}`}
            extra={<Space>
                <Button
...
export const FlashCardSideContent = ({
    type,
    value,
    preview,
    onClick,
    ...
    type: string,
    value: string,
} & Pick<React.ComponentProps<typeof Image>, 'preview'>
    & Pick<React.HTMLAttributes<HTMLDivElement>, 'onClick' | 'className'>) => {
    const imageType = type === FlashCardSideType.IMAGE_URL;

    return <StyledFlashCardSideContent
        $noPadding={imageType}
        className={mergeClass('card-side-content', className)}
        onClick={onClick}
    >
        {imageType
            ? <Image
            ...
            />
            : <span className="card-side-content-text">{value}</span>
        }
    </StyledFlashCardSideContent>;
};
